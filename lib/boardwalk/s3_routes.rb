module Boardwalk
  class App < Sinatra::Base
    ...
    put %r{/([^\/]+?)/(.+)} do
      puts "\e[1;32mLine 40: put %r{/([^\/]+?)/(.+)}\e[0m"

      aws_authenticate

      bucket_name = params[:captures][0]
      file_name = params[:captures][1]
      amz = env['x-amz-acl']

      @user.buckets.each { |b| @bucket = b if b.name == bucket_name }

      slot = @bucket.slots.to_enum.find { |s| s.file_name == file_name }

      File.open("tmp/#{file_name}", "w+") do |f|
        f.print request.body.read
      end

      if slot.nil?
        slot = @bucket.slots.create(
          :bit => open("tmp/#{file_name}"),
          :file_name => file_name,
          :access => amz,
          :created_at => Time.now,
          :updated_at => Time.now
    ...
    delete %r{/([^\/]+?)/(.+)} do
      puts "\e[1;32mLine 74: delete %r{/([^\/]+?)/(.+)}\e[0m"
    ...
    get %r{/([^\/]+)/?} do |e|
      puts "\e[1;32mLine 86: get %r{/([^\/]+)/?}\e[0m"
    ...
    put %r{/([^\/]+)/?} do
      puts "\e[1;32mLine 195: put %r{/([^\/]+)/?}\e[0m"
      puts url
